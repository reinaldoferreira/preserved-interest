...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = store || {},
	    getSingleObject = function getSingleObject(path) {
	  var folder = path !== '/' ? path.substr(0, path.indexOf('/', 1)) : path;

	  return store[folder].reduce(function (acc, val) {
	    path = path === '/' ? '/index.html' : path;
	    if (path === val.id) acc = val;
	    return acc;
	  }, {});
	},
	    setTitle = function (siteName) {
	  siteName = siteName || 'Preserved Interest';
	  return function (title) {
	    document.title = title + ' | ' + siteName;
	  };
	}(),
	    goToPage = function goToPage(url, title) {
	  history.pushState(null, null, url);
	},
	    translateClickToAjax = function translateClickToAjax(e) {
	  var target = e.target.tagName !== 'A' ? e.target.parentNode : e.target;
	  if (target && !!target.getAttribute('data-target')) {
	    var path = target.href.replace(location.origin, ''),
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	request.get('http://localhost:3000/content.json').end(function (err, res) {
